...
def generate(c):
    raise RuntimeError('Read-only reuse: seed7/8 must never generate or modify shared data')
...
def benchmark(c):
    raise RuntimeError('Read-only reuse: benchmark is already complete in seed6')
    import cv2
    cv2.setNumThreads(1)

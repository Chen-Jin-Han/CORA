...
from common import TASKS,DEGS,save_json
BASE=Path('/data1/CST/CORA/promptir-nafnet-baselines');MODELS=['promptir','nafnet']
...
try:
 while True:
  stages={}
  for model in MODELS:
   path=BASE/f'{model}_seed6_b128_s15000_full13_v1'/'status.json'
   stages[model]=json.loads(path.read_text())['stage'] if path.exists() else 'missing'
  if any(x=='failed' for x in stages.values()):raise RuntimeError(f'GPU stage failed: {stages}')
  if all(x=='gpu_complete' for x in stages.values()):break
  status('waiting_gpu_training',model_stages=stages);time.sleep(30)
 for model,gpu in [('promptir',0),('nafnet',2)]:

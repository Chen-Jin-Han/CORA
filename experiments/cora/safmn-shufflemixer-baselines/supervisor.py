...
def main():
    assert os.environ.get('PHASE')=='gpu', 'master.py owns control and report stages'
    ...
    def choose_gpu(model):
        ...
        for line in raw.splitlines():
            ...
            if os.environ.get('ALLOWED_GPU') and str(idx)!=os.environ['ALLOWED_GPU']:continue
    ...
    try:
        ...
        while not all(done('images_'+m) for m in [c['model']]):
            reap()
            for m in [c['model']]:
            ...
            if not all(done('images_'+m) for m in [c['model']]):time.sleep(30)
        status('gpu_complete')

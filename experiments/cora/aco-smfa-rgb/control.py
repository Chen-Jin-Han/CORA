...
from corruptions import VDCS
from engine import load_model, torch_setup
from metrics import uint8


def evaluate(c,task,condition):
    from dreamer_bridge import make_agent,make_env,reset,act
    assert condition in ['clean','raw','aco','smfa']
    torch_setup(c)
    out=root(c)/'control'/task/condition; out.mkdir(parents=True,exist_ok=True)
    signature=dict(config=fingerprint(c),task=task,condition=condition)
    ...
    for ep in range(c['control_episodes']):
        ...
        degrader=VDCS(seed+10000000); score=0.; steps=0; adapter_times=[]
        ...
        row=dict(task=task,condition=condition,seed=seed,return_=score,decisions=steps,

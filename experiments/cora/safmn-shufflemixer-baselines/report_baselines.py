...
def report():
 ...
 for model in MODELS:
  ...
  assert ck['step']==c['steps']==15000 and ck['name']==model and ck['config_hash']==fingerprint(source_config)
 ...
 previous=json.loads(Path('/data1/CST/CORA/promptir-nafnet-frozen6500/results.json').read_text())
 for old in previous['new_methods']:
  assert old['seed']==6
  base.append(old)
  all_episodes[old['method']]=previous['episodes'][old['method']]
 ...
 allrows=base+rows
 lines=['# SAFMN and ShuffleMixer RGB restoration baselines, seed6','','SAFMN and ShuffleMixer use official standard trunks adapted with scale=1 reconstruction heads, trained from scratch for 15000 updates. One seed per adapter. Same 585000/65000/130000 data pairs, batch128, L1+0.05FFT, frozen DreamerV3. All 13 corruption types occurred in training; no OOD claim. Raw/ACO/CORA/PromptIR/NAFNet values are read-only reused seed6 results. PromptIR stopped at 6500 updates; all other adapters use 15000. The PromptIR comparison is not update-budget matched. Fixed-type return averages 650 episodes per adapter; Markov averages 100. All values are single-seed means, with no seed SD.','','## Table 1 · Fixed-type control return','','| Controller | Adapter | Mean return ↑ |','|---|---|---:|']
 ...
 save_json(BASE/'verification.json',dict(complete=True,models=MODELS,seed=6,checkpoint_steps={m:15000 for m in MODELS},image_pairs_each=130000,formal_control_episodes=1500,markov_frame_files=20,markov_frame_positions=50000,baseline_full_reused=True))

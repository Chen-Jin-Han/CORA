...
def load_model(c, name, device):
    ...
    if 'checkpoint_source' in c:
        assert c['seed']==source_config['seed']==6 and c['variant']==source_config['variant']
    ...
    model=build(name,c['variant']).to(device); model.load_state_dict(state['model'],strict=True); model.eval()
    return model
...
def train(c,name,device):
    ...
    model=build(name,c['variant']).to(device)

...
class SMFARGB(nn.Module):
    def __init__(self, variant='full'):
        ...
        self.feats = nn.Sequential(*[FMB(36, variant=variant) for _ in range(8)])
...
def build(name,variant='full'):
    assert name=='smfa', 'This project is CORA ablation only'
    return SMFARGB(variant)

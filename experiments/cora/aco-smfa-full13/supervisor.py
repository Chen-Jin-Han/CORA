...
def main():
    ...
    try:
        wait_stage('benchmark',['run.py','benchmark'],1)
        wait_stage('generate',['run.py','generate'],1)
        # Each model independently queues smoke -> train -> images. No extra seeds.
        while not all(done('images_'+m) for m in ['aco','smfa']):
            reap()
            for m in ['aco','smfa']:
            ...
            if not all(done('images_'+m) for m in ['aco','smfa']):time.sleep(30)
        queue=[]

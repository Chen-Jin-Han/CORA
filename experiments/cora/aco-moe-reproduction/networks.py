...
class ConvEncoder(nn.Module):
    def __init__(
        ...
        for i in range(stages):
            ...
            in_dim = out_dim
            out_dim *= 2
            # Conv2dSamePad with stride=2 yields ceil(H/2), ceil(W/2) spatial sizes.
            h, w = int(math.ceil(h / 2)), int(math.ceil(w / 2))
...
class ConvDecoder(nn.Module):
    def __init__(
        ...
        # Simulate encoder's ceil-based downsampling to find actual minres
        # This matches ConvEncoder which uses ceil(h/2), ceil(w/2)
        h, w = shape[1], shape[2]
        ...
        out_ch = self._minres_h * self._minres_w * depth * 2 ** (layer_num - 1)
        self._embed_size = out_ch

        self._linear_layer = nn.Linear(feat_size, out_ch)
        self._linear_layer.apply(tools.uniform_weight_init(outscale))
        in_dim = out_ch // (self._minres_h * self._minres_w)
        out_dim = in_dim // 2

        layers = []
        h, w = self._minres_h, self._minres_w
    ...
    def forward(self, features, dtype=None):
        ...
        x = x.reshape(
            [-1, self._minres_h, self._minres_w, self._embed_size // (self._minres_h * self._minres_w)]
        )
        # (batch, time, -1) -> (batch * time, ch, h, w)
        x = x.permute(0, 3, 1, 2)
        x = self.layers(x)
        # Interpolate to exact target size if needed (handles non-power-of-2 dimensions)
        target_h, target_w = self._shape[1], self._shape[2]

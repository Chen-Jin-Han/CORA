...
def load_model(c, name, device):
    ...
    if 'checkpoint_source' in c:
        assert c['seed']==source_config['seed']==1
    ...
    assert state['step']==c['steps'] and state['config_hash']==fingerprint(source_config) and state['name']==name
...
def train(c,name,device):
    assert c['seed'] in [2,3], 'seed1 is evaluation-only'

...
def read_config(path):
    ...
    assert c['seed'] == 6 and c['image_size'] == 64
    assert c['model'] in ['promptir','nafnet']
    assert c['frozen_checkpoint_step'] == (6500 if c['model']=='promptir' else 15000)
    assert c['checkpoint_source'].startswith('/data1/CST/CORA/promptir-nafnet-baselines/')

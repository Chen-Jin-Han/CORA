...
def main():
 ...
 base=Path('/data1/CST/CORA/aco-smfa-markov');base.mkdir(parents=True,exist_ok=True)
 ...
 for s in [1,2,3]:bind(read_config(f'config_seed{s}.json'))
 ...
 try:
  for stage in ['preflight','audit']:
   status(stage);finish(runjob(stage,['experiment.py',stage,'--config','config_seed2.json']))
  lanes={n:dict(gpu=g,job=None,queue=[(f'smoke_{n}',['smoke.py','--model',n])]+[(f'train_{n}_seed{s}',['experiment.py','train','--config',f'config_seed{s}.json','--model',n]) for s in [2,3]]) for n,g in [('aco','2'),('smfa','3')]}
  # Seed1 evaluation can proceed while the new networks train; at most two CPU jobs.
  queue=[(f'control_s{s}_{t}_{n}',['experiment.py','control','--config',f'config_seed{s}.json','--task',t,'--condition',n]) for s in [1,2,3] for t in read_config('config_seed1.json')['tasks'] for n in (['clean','raw','aco','smfa'] if s==1 else ['aco','smfa'])]
  cpu=[]
  while lanes or queue or cpu:
   ...
   for item in list(queue):
    ...
    queue.remove(item);j=runjob(key,args,dict(os.environ,CUDA_VISIBLE_DEVICES='',MUJOCO_EGL_DEVICE_ID='2'))
    if j:cpu.append(j)
   status('running',queued_controls=len(queue),gpu_lanes=list(lanes),poll_seconds=30)
   if lanes or queue or cpu:time.sleep(30)
  finish(runjob('report',['summarize.py']));status('complete',episodes=800)

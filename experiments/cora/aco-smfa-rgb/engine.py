...
def load_model(c, name, device):
    state=torch.load(root(c)/'checkpoints'/name/'last.pt',map_location='cpu',weights_only=False)
    assert state['step']==c['steps'] and state['config_hash']==fingerprint(c) and state['name']==name
...
def train(c,name,device):
    ...
    trainset=Pairs(c,'train'); valset=Pairs(c,'val')
    assert len(trainset)==45000 and len(valset)==5000
    ...
    for step in range(start+1,c['steps']+1):
        ...
        ids=rng.choice(len(trainset),c['batch_size'],replace=False)
        ...
        while True:
            ...
            for pos in range(0,c['batch_size'],c['micro_batch']):
                ...
                with torch.autocast('cuda',enabled=c['amp']):
                    pred=model(x); loss=(pred-y).abs().mean()
        ...
        if step%c['log_every']==0:
            row=dict(step=step,train_l1=loss_sum,elapsed=elapsed+time.perf_counter()-begin)
            history.append(row); print(json.dumps(row),flush=True)
        if step%c['validate_every']==0 or step==c['steps']:
            val=validate(c,model,valset,device)

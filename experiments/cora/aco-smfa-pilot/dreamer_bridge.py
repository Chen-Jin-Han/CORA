...
def foreground(dm, size, camera):
    import mujoco
    seg = dm.physics.render(*size, camera_id=camera, segmentation=True)
    ids, kind = seg[..., 0], seg[..., 1]
    valid = (kind == int(mujoco.mjtObj.mjOBJ_GEOM)) & (ids >= 0)
    mask = np.zeros(ids.shape, bool)
    # Non-world geoms plus task target sites. Finger's target is a world-body SITE.
    mask[valid] = np.asarray(dm.physics.model.geom_bodyid)[ids[valid]] != 0
    site = (kind == int(mujoco.mjtObj.mjOBJ_SITE)) & (ids >= 0)
    target_ids = [i for i in range(dm.physics.model.nsite)
                  if dm.physics.model.id2name(i, 'site') in ('target', 'tip')]
    mask |= site & np.isin(ids, target_ids)
    return mask

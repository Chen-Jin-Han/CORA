...
class Logger:
    def __init__(self, logdir, step, *, wandb_run=None):
        ...
        self._videos = {}
        self.step = step
        self.last_eval_return = None  # Track the latest eval return for best model saving
    ...
    def write(self, fps=False, step=False):
        ...
        if self._wandb_run is not None and scalars:
            try:
                metrics = {k: v for k, v in scalars}
                if "train_return" in metrics and "train_reward" not in metrics:
                    metrics["train_reward"] = metrics["train_return"]
                if "eval_return" in metrics and "eval_reward" not in metrics:
                    metrics["eval_reward"] = metrics["eval_return"]
                self._wandb_run.log(metrics, step=step)
            except Exception as exc:
                print(f"[WARN] W&B log failed: {exc}")
            ...
            for i in indices:
                ...
                if not is_eval:
                    ...
                    if len(eval_scores) >= episodes and not eval_done:
                        ...
                        logger.last_eval_return = score  # Save for best model tracking
                        logger.write(step=logger.step)
                        eval_done = True
                        final_eval_return = score
    if is_eval:
        # keep only last item for saving memory. this cache is used for video_pred later
        while len(cache) > 1:
            # FIFO
            cache.popitem(last=False)
    result = (step - steps, episode - episodes, done, length, obs, agent_state, reward)
    if is_eval and 'final_eval_return' in locals():
        return result, final_eval_return
    return result
...
class Optimizer:
    def __init__(
        ...
        use_scaler = bool(use_amp and (amp_dtype is None or amp_dtype == torch.float16))
        self._scaler = torch.amp.GradScaler("cuda", enabled=use_scaler)
    ...
    def __call__(self, loss, params, retain_graph=True):
        ...
        if not torch.isfinite(loss.detach()) or not torch.isfinite(norm.detach()):
            metrics[f"{self._name}_skipped"] = 1.0
            self._scaler.update()
            self._opt.zero_grad()
...
def args_type(default):
    def parse_string(x):
        ...
        if isinstance(default, (list, tuple)):
            # Handle Python list/tuple string representation (e.g., "['a', 'b']" or "('a', 'b')")
